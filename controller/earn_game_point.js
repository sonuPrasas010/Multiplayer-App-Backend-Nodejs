...
const collectGamePointSpin = async (req = request, res = response) => {
  const ip = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
  console.log(ip); // ip address of the user
  const userInfo = (lookup("2402:6640:17:9115:45ce:40f:34ea:66e7")); // location of the user
...
const getGamePointScratch = async(req = request, res = response) => {
  const ip = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
  console.log(ip); // ip address of the user
  const userInfo = (lookup("2402:6640:17:9115:45ce:40f:34ea:66e7")); // location of the user
...
const collectGamePointScratch = async (req = request, res = response) => {
  const ip = req.headers["x-forwarded-for"] || req.socket.remoteAddress;
  console.log(ip); // ip address of the user
  const userInfo = (lookup("2402:6640:17:9115:45ce:40f:34ea:66e7")); // location of the user
...
module.exports = {
  ...
  getGamePointSpin,
  collectGamePointSpin,
  getGamePointScratch,
  collectGamePointScratch
}
